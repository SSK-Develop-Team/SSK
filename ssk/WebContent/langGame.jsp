...
<%@ page import="util.lang.LangGameProcessor" %>
...
<%
	...
	if(currLangGameElement.getLangGameHint()==null&&currLangGameElement.getLangGameHintVoice()==null&&currLangGameElement.getLangGameAnswer()==null&&currLangGameElement.getLangGameAnswerVoice()==null&&currLangGameElement.getLangGameResultInputFlag()==false){
...
%>
<title><%= dontKnowList.get(currDontKnowIndex)%>번 문항 직접 평가</title>
</head>
<body>
<%@ include file="sidebar.jsp" %>
<h2>직접평가 #<%= dontKnowList.get(currDontKnowIndex)%></h2>
<div class="w3-row">
...
<div class="w3-col w3-container s12 m8 l6">
	...
	<div class="w3-container w3-round-large w3-padding" style="border:1px solid #12192C;">
		...
		if(currLangGameElement.getLangGameResultInputFlag()){%>
			<button class="w3-button" onclick="alert('아이의 답변을 듣고 O,X를 입력해주세요.');" style="border:none; background-color:#FFFFFF;">다음 &gt; </button>
		<%}else if(i<=langGameList.size()-1){%>
			<button class="w3-button" onclick="getNextContent(<%=i%>, <%= dontKnowList.get(currDontKnowIndex)%>,<%=langGameList.size()%>);" style="border:none; background-color:#FFFFFF;">다음 &gt; </button>
		<%}%>
		</div>
	</div>
	...
	<div class="w3-col w3-right" style="width:35%;">
		<div class="w3-row">
			<div class="w3-container w3-third" onclick="inputResult('1', <%=i%>, <%= dontKnowList.get(currDontKnowIndex)%>,<%=langGameList.size()%>);"><img src="./image/OButton.png" style="width:100%;"/></div>
			<div class="w3-container w3-third" onclick="inputResult('0', <%=i%>, <%= dontKnowList.get(currDontKnowIndex)%>,<%=langGameList.size()%>);"><img src="./image/XButton.png" style="width:100%;"/></div>
			<div class="w3-container w3-third" onclick="alert('?를 클릭하셨습니다.');"><img src="./image/QButton.png" style="width:100%;"/></div>
		</div>
	</div>
	<%} %>	
	</div>
...
window.onload = function () {
	...
	if(<%=audioEndedNextFlag%>==1){
		audio.addEventListener("onended",function(){
			alert("end");
			getNextContent(<%=i%>,<%=dontKnowList.get(currDontKnowIndex)%>,<%=langGameList.size()%>);
		});
	}
	audio.play();
}
