	  		...
	  		<c:choose>
				...
				<c:when test="${user.userRole eq 'CHILD'}">
		  			<form id="f" name = "post" action = "logout">
		  				...
		  				<a class="w3-bar-item w3-button w3-hover-none w3-text-black w3-hover-text-blue w3-padding-right w3-right w3-margin-right w3-margin-top"style="font-size:1.5rem;" onclick="location.href='../EwhaSSK/childHome.jsp'">아동 ${user.userName}&nbsp;님</a>				
		  			</form>				
				</c:when>
				<c:when test="${user.userRole eq 'EXPERT'}">
		  			<form id="f" name = "post" action = "logout">
			  			<button class="w3-bar-item w3-button w3-text-white w3-hover-text-black w3-padding-right w3-right w3-round-xlarge w3-margin-top" style="background-color:#1A2A3A;font-size:1.2rem;margin-right:30px;" href="javascript:void(0);" onclick="doLogout();">로그아웃</button>
		  				<a class="w3-bar-item w3-button w3-hover-none w3-text-black w3-hover-text-blue w3-padding-right w3-right w3-margin-right w3-margin-top"style="font-size:1.5rem;" onclick="location.href='../EwhaSSK/GetExpertHome'">전문가 ${user.userName}&nbsp;님</a>				
		  			</form>				
				</c:when>
	  			<c:otherwise>
		  			<form id="f" name = "post" action = "logout">
			  			<button class="w3-bar-item w3-button w3-text-white w3-hover-text-black w3-padding-right w3-right w3-round-xlarge w3-margin-top" style="background-color:#1A2A3A;font-size:1.2rem;margin-right:30px;" href="javascript:void(0);" onclick="doLogout();">로그아웃</button>
		  				<a class="w3-bar-item w3-button w3-hover-none w3-text-black w3-hover-text-blue w3-padding-right w3-right w3-margin-right w3-margin-top"style="font-size:1.5rem;" onclick="location.href='../EwhaSSK/GetAdminHome'">${user.userName}&nbsp;님</a>				
		  			</form>
	  			</c:otherwise>
	  		</c:choose>
